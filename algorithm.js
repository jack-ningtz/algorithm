...
function shell_sort(arr) {
    for (let gap = parseInt(String(arr.length / 2)); //Math.floor(arr.length/2);
     gap > 0; gap = parseInt(String(gap / 2))) {
...
function merge_sort_up2down(arr) {
    let len = arr.length;
    if (len < 2)
        return arr;
    ...
    return merge(merge_sort_up2down(left), merge_sort_up2down(right) //对左右子串再拆分，直到只有一个元素
    );
}
function merge_sort_down2up(arr) {
    let _arr = arr;
    let result;
...
//*************** heap sort ******************/
//最大堆
function heap_tree_down(arr, start, end) {
    let current = start;
...
function heap_tree_up(arr, start) {
    let c = start;
...
function radix_sort(arr) {
    ...
    for (let p = 0; p < place; p++) {
        ...
        for (let i = 0; i < arr.length; i++) {
            let radix = Math.floor(arr[i] / (Math.pow(10, p))) % 10;
